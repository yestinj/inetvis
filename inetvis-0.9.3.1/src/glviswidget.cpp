...
GLVisWidget::GLVisWidget(QWidget *parent, char const *name) : QGLWidget(parent, name)
{
   ...
   recFrameCount = 0;
   fpsFrameCount = 0;
   lastTime.tv_sec = 0; //init lastTime
   lastTime.tv_usec = 0;
   fps = 0;
...
void GLVisWidget::keyReleaseEvent(QKeyEvent *ev)
{
   ...
   {  case Qt::Key_F: //toggle full screen
         if (ev->state() & Qt::ControlButton) //ctrl F
         {  setFrontView();
         }
         else // just F
         ...
         break;
      case Qt::Key_L:
         if (ev->state() & Qt::ControlButton) //ctrl L
         {  setLeftView();
         }
         break;
      case Qt::Key_T:
         if (ev->state() & Qt::ControlButton) //ctrl T
         {  setTopView();
         }
         break;
      case Qt::Key_B:
         if (ev->state() & Qt::ControlButton) //ctrl B
         {  setBottomView();
         }
         break;
      ...
      case Qt::Key_R: //toggle control panel
         if (ev->state() & Qt::ControlButton) //ctrl R
         {  setRightView();
         }
         else
         ...
         break;
      case Qt::Key_H:
         if (ev->state() & Qt::ControlButton) //ctrl H
         {  if (hideHomeRange)
...
{
   //calcultate time lapse in seconds
   struct timeval currTime;
   gettimeofday(&currTime, NULL);
   float timeLapse = (currTime.tv_sec - lastTime.tv_sec)
         + ((currTime.tv_usec - lastTime.tv_usec)/1000000.0);
   //calculate the fps
   fps = (int)(fpsFrameCount / timeLapse);
   ...
   lastTime.tv_sec = currTime.tv_sec; //get current time
   lastTime.tv_usec = currTime.tv_usec;

   //NOTE: we could use the fpsSampleRate rather than getting time values,
   //but the accuracy would rely on timeous timer calls, and under heavy system
   //load, there may be a delay
