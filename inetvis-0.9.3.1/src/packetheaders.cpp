                                ...
                                int length)
{
   ...
   {  //note we reley on the caller that the capture was big enough before
      ...
      {  packet.networkType = N_IPv4;
         ...
         //temporary solution - fragmenets are filtered out with BPF
...
void PacketHeaders::coutHeaders(struct OSI_Packet &packet)
{
   ...
   {  //datalink layer
      ...
      {  struct IP_Header *ip;
         ip = (struct IP_Header*)packet.pt_networkHeader;
         cout << "network    > IPv4: src = " << ipAdrToDecDotStr(ip->srcAddress)
              << " (" << (ip->srcAddress) << "), dst = "
              << ipAdrToDecDotStr(ip->dstAddress) << " (" << (ip->dstAddress)
